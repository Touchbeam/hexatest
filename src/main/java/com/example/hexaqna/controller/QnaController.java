...
public class QnaController {
    ...
    public Map<String, String> checkPw(@PathVariable("qno") Long qno , @RequestBody PwForm pwForm) {
        log.info("비번 {}", pwForm.getPassword());
        Qna qna = qnaRepository.findById(qno).orElseThrow(); ;
        boolean isAuthenticated = (qna.getPassword().equals(pwForm.getPassword()));

        if (isAuthenticated) {
            log.info("비번같음 {}", pwForm.getPassword());
            return Map.of("success", "success");
        } else {
            log.warn("비번다름 {}", pwForm.getPassword());
